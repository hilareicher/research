...
from date_regex import (HEB_FULL_DATE_REGEX,
...
class DateMention:
    ...
    def reconstruct_date_string(self) -> str:
        ...
        date_components = [dt for dt in date_components if
                           (dt is not None) and (dt.text is not None)]  # filter empty date components
...
def set_date_mention_component(matched: re.Match, ind: Union[int, str]) -> Optional[DateMentionComponent]:
    ...
    '''
    if type(ind) == int:
        date_instance = DateMentionComponent(matched.group(ind), matched.span(ind)[0],
                                             matched.span(ind)[1]) if ind > 0 else None

    else:
        date_instance = DateMentionComponent(matched.group(ind), matched.span(ind)[0],
                                             matched.span(ind)[1]) if ind in matched.groupdict() else None
...
def extract_date_components(text: str) -> DateMention:
    ...
    pattern_list = [HEB_FULL_DATE_REGEX, HEB_MONTH_YEAR_REGEX, HEB_DAY_MONTH_REGEX,
                    ...
                    EN_DATE_REGEX, SPLIT_BY_SPACE_REGEX]
